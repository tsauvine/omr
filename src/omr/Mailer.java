...
public class Mailer {
    ...
    public void sendFeedback(Project project, Sheet sheet) throws Exception {
        ...
        String host = OMRProperties.getString("smtp-host", "specify host in property file");
        String from = OMRProperties.getString("from-address", "specify from in property file");
        String to = OMRProperties.getString("address-prefix", "" ) +
                sheet.getStudentId() + OMRProperties.getString("address-suffix", "");

        Properties sysProps = System.getProperties();
        sysProps.put("mail.smtp.host", host);

        // Get session
        Session session = Session.getDefaultInstance(OMRProperties.values, null);
        ...
        message.setSubject(OMRProperties.getString("subject", "Exam Result" ));
